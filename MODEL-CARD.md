...
Kiku's checkpoints are OpenAI's Whisper weights (MIT-licensed), loaded into
our own Rust implementation of the architecture. This card ports the
substance of upstream's `model-card.md` to the facts that matter for
Molterra, and states what our runtime adds and what it does not change.
...
| large (v1–v3) | 1550 M | yes | best accuracy |
...
to translate — use a full multilingual checkpoint for X→English.

## Training data and language coverage

The weights were trained on 680,000 hours of weakly-supervised web audio
(~117k hours non-English across ~98 languages, ~125k hours X→English). Per
the paper's Appendix D, accuracy varies widely by language: high-resource
...
## Known limitations (upstream's, inherited by any faithful port)
...
- **Hallucination.** A sequence-to-sequence decoder can emit text that was
  ...
  noise. Kiku counters with the paper's VAD rule, the compression-ratio
...
Same as upstream: no surveillance of individuals without consent, no
